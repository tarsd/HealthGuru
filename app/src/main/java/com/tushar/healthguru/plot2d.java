...
public class plot2d extends View {
	...
	public plot2d(Context context, float[] xvalues, float[] yvalues,String[] date,int number, int axes) {
		super(context);
			...
			else
		   {
			...
			for (int i=1;i<=n;i++){
				try {
					...
					canvas.drawText(date[i - 1], (float) toPixelInt(canvasWidth, minx, maxx, temp), canvasHeight - locxAxisInPixels + 20, paint);
			...
			{
             //  Toast.makeText(v.getContext(),"Insufficient Data! Please Enter a Lower Value",Toast.LENGTH_LONG).show();
                //Toast.makeText(v.getContext(),""+number,Toast.LENGTH_LONG).show();
                Log.e("Error ",e+"");

			}}
			canvas.drawText(""+date[number-1], (float)toPixelInt(canvasWidth, minx, maxx, maxx),canvasHeight-locxAxisInPixels+20, paint);
